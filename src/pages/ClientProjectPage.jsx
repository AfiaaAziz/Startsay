...
import {
  ...
  getAdjacentClientProjects,
} from "../data/clientProjects";

gsap.registerPlugin(ScrollTrigger);

export default function ClientProjectPage({ isContactOpen, setIsContactOpen }) {
  ...
  // Fetch Data
  useEffect(() => {
    const data = getClientProjectBySlug(projectSlug);

    if (data) {
      setProject(data);
      setGalleryImages(data.gallery || []);

      const { previous, next } = getAdjacentClientProjects(projectSlug);
            ...
            <div className="w-dyn-list">
              {previous && (
                <div id="listPrev" role="list" className="w-dyn-items">
                  ...
                  >
                    <Link
                      to={`/client-project/${previous.slug}`}
